...
prerequisites: ["Basic knowledge of divisibility"]
---
...
The core idea behind modular arithmetic is remainders, and performing operaions on them. For example, we know that $43$ and $46$ have the same remainder when divided by $3$. Now observe that $43=1+3(14)$ and $46=1+3(15)$(This is the euclidean division lemma which will be discussed later), so $3$ divides $46-43$. And based on the format we have used to represent them, this is true for any two numbers which have the same remainder when divided by a third number.

Thus we can say $46$ and $43$ are similar or **equivalent** in some way under a certain condition. And this is where modular arithmetic comes into the picture

Definition: We say that $a\equiv b\;(mod\:d)$ if $d\mid{a-b}$

Even though saying that a and b has the same remainder when divided by d is more intuitive and usually comfortable, the idea of $d\mid{a-b}$ and the skill fo identifying it is crucial in the AMC 10/12.
